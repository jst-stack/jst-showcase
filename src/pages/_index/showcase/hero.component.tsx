...
function InstallActions({ copied, onCopy }: { copied: boolean, onCopy: () => void }) {
	return (
		<div className="mx-auto mt-8 flex max-w-lg flex-col items-stretch gap-3 sm:flex-row sm:justify-center">
			<div className="flex items-center gap-3 rounded-md border border-border bg-muted/50 px-4 py-2.5">
				<span aria-hidden="true" className="select-none font-mono text-sm text-muted-foreground">$</span>
				<code className="flex-1 truncate text-left font-mono text-sm text-foreground">{INSTALL}</code>
				<button type="button" onClick={onCopy} className="text-muted-foreground transition-colors hover:text-foreground" aria-label="Copy install command">
					{copied ? <Check className="size-4 text-primary" aria-hidden="true" /> : <Copy className="size-4" aria-hidden="true" />}
				</button>
			</div>
			<a href="https://github.com/jst-stack/jst" className="inline-flex items-center justify-center gap-2 rounded-md bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground transition-opacity hover:opacity-90">
		...
		<div className="mx-auto mt-12 max-w-md overflow-hidden rounded-lg border border-border bg-card text-left shadow-sm">
			<div className="relative flex items-center border-b border-border bg-muted/60 px-4 py-2.5">
				<div className="flex items-center gap-1.5">
					{[1, 2, 3].map(dot => <span key={dot} className="size-3 rounded-full bg-border" aria-hidden="true" />)}
				</div>
				<span className="absolute left-1/2 -translate-x-1/2 font-mono text-xs text-muted-foreground">jst — setup</span>
			</div>
			<div className="space-y-1 px-5 py-5 font-mono text-[13px] leading-relaxed">
				...
				<p className="term-line text-foreground" style={{ animationDelay: '0.5s' }}>Creating my-app…</p>
				...
				<p className="term-line text-muted-foreground/70" style={{ animationDelay: '1.3s' }}>Installing dependencies and initializing Git…</p>
				<p className="term-line flex items-center gap-1.5 pt-2 text-foreground" style={{ animationDelay: '1.7s' }}>
