				...
				<div className="mt-12 grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] lg:gap-10">
					...
					<div>
						<div className="relative pl-6">
							<div
								className="pointer-events-none absolute left-[7px] top-5 bottom-5 w-px overflow-hidden bg-gradient-to-b from-primary/70 via-primary/40 to-primary/10"
								aria-hidden="true"
							>
								...
								{layers.map((layer, i) => {
									...
									return (
										<li key={layer.id} className="relative">
											<span
												className={cn(
													'pointer-events-none absolute -left-[calc(1.5rem-4px)] top-1/2 size-2 -translate-y-1/2 rounded-full border-2 transition-colors',
													isActive
														? 'animate-node border-primary bg-primary'
											...
											<button
												...
												className={cn(
													'group flex w-full items-center gap-4 rounded-lg border px-4 py-4 text-left transition-all',
													isActive
						...
						<p className="mt-4 pl-6 font-mono text-xs text-muted-foreground">
							→ imports are allowed only down the stack
						</p>
					</div>
					...
					<div className="min-w-0">
						...
						<p className="mt-4 text-pretty text-sm leading-relaxed text-muted-foreground">
							...
							{' '}
							only when it
							is independent of a domain context and has proven reuse—not because its owner is still unclear.
						</p>
					</div>
				</div>
			</div>
